...
/**
 * Independent Product Card Component with Multiple Layouts
 * Each card manages its own cart loading state and API calls
 * Supports both column (vertical) and row (horizontal) layouts
 */
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  // Row Layout (New Horizontal Layout)
  return (
    <Card 
      className={cn(
        "group relative overflow-hidden border-0 shadow-md bg-white transition-all duration-500 hover:shadow-2xl hover:shadow-primary/10 h-full",
    ...
    >
      <div className="flex h-full">
        ...
        <div className="relative overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100 w-1/2 min-h-[280px]">
          <img
            src={getProductImage(product, selectedVariant, '400x400')}
            alt={getProductImageAlt(product, selectedVariant)}
          ...
          {/* Quick Actions Overlay - Top Right of Image */}
          {showQuickActions && (
            <div className={cn(
              "absolute top-4 right-4 flex flex-col gap-2 transition-all duration-300",
              isHovered ? "opacity-100 translate-x-0" : "opacity-0 translate-x-4"
              ...
              <Button
                size="sm"
                variant="secondary"
                className="w-8 h-8 rounded-full bg-white/95 backdrop-blur-sm hover:bg-white shadow-lg border-0"
                onClick={() => setIsLiked(!isLiked)}
              >
                <Heart className={cn(
          ...
          <div className="absolute top-4 left-4 flex flex-col gap-1">
            {isProductOnSale(selectedVariant) && (
              <Badge className="bg-red-500 text-white font-bold shadow-lg border-0 animate-pulse text-xs">
                -{discountPercentage}%
              </Badge>
            )}
            {!selectedVariant.available && (
        ...
        <div className="w-1/2 flex flex-col">
          <CardContent className="p-4 flex flex-col h-full space-y-3">
            ...
            {/* Title */}
            <h3 className="font-bold line-clamp-2 group-hover:text-primary transition-colors leading-tight text-sm">
              {product.title}
            </h3>
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-1">
                {selectedVariant.available ? (
                  <Badge variant="secondary" className="bg-green-100 text-green-700 text-xs border-0 px-2 py-0">
                    ✓ In Stock
                  </Badge>
                ) : (
                  <Badge variant="secondary" className="bg-red-100 text-red-700 text-xs border-0 px-2 py-0">
                    ✗ Out of Stock
                  </Badge>
                )}
                {isProductOnSale(selectedVariant) && (
                  <Badge className="bg-orange-100 text-orange-700 text-xs border-0 px-2 py-0">
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  <select
                    value={selectedVariantId}
                    onChange={handleVariantChange}
                    className="w-full px-2 py-1.5 text-xs border border-gray-200 rounded-md bg-white appearance-none cursor-pointer focus:outline-none focus:ring-1 focus:ring-primary/50 focus:border-primary transition-all"
                  >
                    {product.variants.map((variant) => (
                      <option key={variant.id} value={variant.id}>
                        {getVariantTitle(variant)} - {formatMoney(variant.price)}
            ...
            <div className="space-y-2 mt-auto">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-xs font-semibold min-w-[1.5rem] text-center bg-gray-50 px-2 py-1 rounded">
                    {quantity}
                  </span>
              ...
              <Button
                onClick={handleAddToCart}
                disabled={!selectedVariant.available || isThisVariantAdding}
                className="w-full h-9 bg-primary hover:bg-primary/90 text-white font-semibold rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 border-0 text-xs"
              >
                {isThisVariantAdding ? (
                  <div className="flex items-center gap-1">
            ...
            {product.tags.length > 0 && (
              <div className="flex flex-wrap gap-1 pt-2 border-t border-gray-100">
                {product.tags.slice(0, 2).map((tag: string) => (
                  <Badge key={tag} variant="outline" className="text-xs border-gray-200 text-gray-600 px-1 py-0">
                    {tag}
                  </Badge>
                ))}
                {product.tags.length > 2 && (
                  <Badge variant="outline" className="text-xs border-gray-200 text-gray-500 px-1 py-0">
                    +{product.tags.length - 2}
                  </Badge>
                )}
              </div>
